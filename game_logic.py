...
def merge(row):
    ...
    for i in range(3):
        if row[i]==row[i+1] and row[i]!=0:            #if two tiles side-by-side have same non zero value, they are merged
            row[i] *= 2
            gain += row[i]        # add only merged tile value to score
            row[i+1] = 0
    return row, gain
...
def move_left(board):
    ...
    for row in board:
        row = slide(row)
        row, g = merge(row)      # collect gained score
        total_gain += g
        row = slide(row)
        result.append(row)
...
def move_board(board, key):
    ...
    new = [r[:] for r in board]              #create a copy of the board
    for i in range(d):
        new = rotate(new)

    new,gain = move_left(new)    # get score from merge only
